...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="bg-[#0c0c10]/95 backdrop-blur-xl border border-white/[0.06] rounded-2xl p-4 shadow-2xl shadow-black/60">
      <p className="text-white/30 text-[11px] font-medium mb-2">{data.label} (todos os anos)</p>
      <div className="flex items-center justify-between gap-6">
        <span className="text-white/40 text-xs">Resultado Acumulado</span>
      ...
      <div className="flex items-center justify-between gap-6 mt-1">
        ...
        <span className="text-white text-xs font-medium">{data.winRate}%</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="relative overflow-hidden bg-white/[0.02] backdrop-blur border border-white/[0.04] rounded-2xl p-6">
      ...
      <div className="flex items-center gap-3 mb-6 relative">
        ...
        <span className="text-white/15 text-[10px] ml-2 uppercase tracking-wider">acumulado de todos os anos</span>
      </div>
