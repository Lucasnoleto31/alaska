...
function StatCard({ label, value, icon: Icon, iconColor }) {
  return (
    <div className="relative overflow-hidden bg-white/[0.02] backdrop-blur border border-white/[0.04] rounded-2xl p-5 flex flex-col gap-4 hover:border-[#00ff87]/20 transition-all duration-300 group">
      ...
      <div className="flex items-center justify-between relative">
        <span className="text-white/30 text-[11px] font-semibold uppercase tracking-[0.15em]">{label}</span>
        <Icon className={`w-4 h-4 ${iconColor}`} />
      </div>
      <div className="text-[22px] font-bold tracking-tight relative">{value}</div>
    </div>
  )
}
