...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="bg-[#0c0c10]/95 backdrop-blur-xl border border-white/[0.06] rounded-2xl p-3 md:p-4 shadow-2xl shadow-black/60">
      <p className="text-white/30 text-[10px] md:text-[11px] font-medium mb-2">{data.date} &middot; {data.time}</p>
      <div className="flex flex-col gap-1">
        <div className="flex items-center justify-between gap-4 md:gap-6">
          <span className="text-white/40 text-[11px] md:text-xs">Trade</span>
          <span className={`text-xs md:text-sm font-bold ${data.profit >= 0 ? 'text-[#00ff87]' : 'text-[#ff4757]'}`}>
            {data.profit >= 0 ? '+' : ''}R$ {data.profit.toLocaleString('pt-BR')}
          </span>
        </div>
        <div className="flex items-center justify-between gap-4 md:gap-6">
          <span className="text-white/40 text-[11px] md:text-xs">Acumulado</span>
...
export default function EquityChart({ data }) {
  ...
  return (
    <div className="relative overflow-hidden bg-white/[0.02] backdrop-blur border border-white/[0.04] rounded-2xl p-4 md:p-6">
      ...
      <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6 relative flex-wrap">
        ...
        {data.length > 0 && (
          <div className="ml-auto flex items-center gap-2 md:gap-3">
            ...
            <span className={`text-sm md:text-lg font-bold ${isPositive ? 'text-[#00ff87]' : 'text-[#ff4757]'}`}>
      ...
      {data.length === 0 ? (
        <div className="h-[250px] md:h-[380px] flex flex-col items-center justify-center gap-2">
