...
function FilterRow({ title, children }) {
  return (
    <div className="flex items-center gap-4">
      <span className="text-white/20 text-[10px] uppercase tracking-[0.2em] font-semibold w-20 shrink-0 text-right">{title}</span>
...
export default function Filters({ filters, setFilters }) {
  ...
  // Toggle para arrays: adiciona se nao tem, remove se ja tem
      ...
      <div className="flex items-center justify-between mb-6 relative">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-[#00ff87]/10 flex items-center justify-center">
            <SlidersHorizontal className="w-4 h-4 text-[#00ff87]" />
          </div>
          <span className="text-white font-semibold text-sm">Filtros</span>
          {hasFilters && (
            <span className="text-[10px] text-[#00ff87] bg-[#00ff87]/10 px-2 py-0.5 rounded-full font-semibold">
        ...
        {hasFilters && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-white/30 hover:text-[#00ff87] transition-all cursor-pointer hover:bg-[#00ff87]/5 px-3 py-2 rounded-xl"
          >
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-white/[0.03] border border-white/[0.06] text-white text-xs rounded-xl pl-9 pr-3 py-2.5 focus:outline-none focus:border-[#00ff87]/40 transition-all cursor-pointer hover:border-white/10 [color-scheme:dark]"
                />
              </div>
        ...
        <FilterRow title="Mes">
          {MONTHS.map((m) => (
            <Btn
              key={m.value}
              label={m.label}
              active={filters.months.includes(m.value)}
              onClick={() => toggleMulti('months', m.value)}
            />
          ))}
        </FilterRow>
        ...
        <FilterRow title="Dia">
          {WEEKDAYS.map((d) => (
            <Btn
              key={d.value}
              label={d.label}
              active={filters.weekdays.includes(d.value)}
              onClick={() => toggleMulti('weekdays', d.value)}
            />
          ))}
        </FilterRow>
        ...
        <FilterRow title="Horario">
          {HOURS.map((h) => (
            <Btn
              key={h.value}
              label={h.label}
              active={filters.hours.includes(h.value)}
              onClick={() => toggleMulti('hours', h.value)}
            />
          ))}
        </FilterRow>
      </div>
